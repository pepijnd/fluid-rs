...
struct SpatialMap {
    data: HashMap<(usize, usize), Vec<usize>>,
    res: f32,
    rows: usize,
    cols: usize,
}
...
impl SpatialMap {
    fn new(max: (f32, f32)) -> SpatialMap {
        let res= 1f32/(max.0 * max.1 / 100000f32);
        let rows = (max.1 * res).ceil() as usize;
        let cols = (max.0 * res).ceil() as usize;
        let data: HashMap<(usize, usize), Vec<usize>> = HashMap::new();
        SpatialMap { data, res, rows, cols }
    }

    fn push(&mut self, x: f32, y: f32, value: usize) {
        let index = self.calc_index(x, y);
        self.data.entry(index).and_modify(|e| e.push(value)).or_insert_with(|| Vec::new());
    }
    ...
    fn get_near(&self, x: f32, y: f32, range: f32) -> Vec<usize> {
        ...
        for xx in ix - range..ix + range {
            ...
            for yy in iy - range..ix + range {
                ...
                let values = &mut self.data.get(&index);
                if values.is_some() {
                    near.append(&mut values.unwrap().clone());
                }
            }
        }
